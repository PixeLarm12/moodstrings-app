...
class KNNService:
    ...
    def train_models(self):
        """Treina o modelo KNN para emoções"""
        ...
        print("Treinando modelo de emoção com KNN...")
        start_time = time.time()
        self._emotion_model.fit(X_train, y_emotion_train)
        print(f"Modelo KNN de emoção treinado em {time.time() - start_time:.2f} segundos")
    ...
    def evaluate_model(self):
        ...
        print(f"\n=== AVALIAÇÃO MODELO KNN DE EMOÇÃO ===")
    ...
    def save_model(self, model_path: str = None):
        ...
        if model_path is None:
            model_path = os.path.join(MODELS_DIR, "emotion_knn_model.pkl")
